        ...
        private void btnFinish_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (MainForm.currentUser != null)
                {
                    ...
                    int PostID = (tmpdb.CreatePost(MainForm.currentUser.UserID, txtTitle.Text));
                    int contentnum = 0;
                    int imagenum = 0;
                    for (int i = 0; i < ImageorContent.Count; i++)
                    {
                        if (ImageorContent[i])
                        {
                            tmpdb.CreatePostContent(PostID.ToString(), PreviewContent[contentnum], i.ToString());
                            contentnum++;
                        }
                        else
                        {
                            tmpdb.CreatePostImage(PostID.ToString(), PreviewImages[imagenum], i.ToString());
                            imagenum++;
                        }
                    }
                }
